...
import { useRouter } from "next/navigation"; // <- Routing walata gaththa
import { useCursorStore } from "@/store/useCursorStore";
import { projectsData } from "@/data/projects"; // <- Ape aluth data file eka link kara

export default function Projects() {
  const [hoveredProject, setHoveredProject] = useState<number | null>(null);
  const { setCursor, resetCursor } = useCursorStore();
  const router = useRouter(); // Next.js Router eka
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 py-24 md:py-40 relative" id="work">
      ...
      <motion.div
        className="hidden md:block fixed top-0 left-0 w-80 h-60 rounded-2xl overflow-hidden pointer-events-none z-50 shadow-2xl shadow-purple-500/20"
        style={{
          x: springX,
          y: springY,
        }}
      ...
      <div className="flex flex-col gap-10">
        {projectsData.map((project, index) => {
          ...
          return (
            <motion.div
              key={project.id}
              // Card Click Routing eka & Cursor Reset eka
              onClick={() => {
                resetCursor();
            ...
            >
              ...
              <div className="flex justify-between items-start z-10">
                <div>
                  ...
                  {/* Dan api data eken ganne "brief" eka, descriptions diga wadi nisa */}
